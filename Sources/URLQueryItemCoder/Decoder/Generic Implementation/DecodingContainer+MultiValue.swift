...
extension DecodingContainer {
    final class MultiValue {
        ...
        private(set) var children: [String: DecodingContainer<PrimitiveValue>]
        ...
        func set(_ key: String, to child: DecodingContainer<PrimitiveValue>) {
...
extension DecodingContainer.MultiValue: KeyedDecodingContainerProtocol {
    ...
    func contains(_ key: StringCodingKey) -> Bool {
        let encodedKey = configuration.keyStrategy.encode(key, at: codingPath)

        return children[encodedKey.stringValue] != nil
    }

    func decode(_ type: Bool.Type, forKey key: StringCodingKey) throws -> Bool {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: String.Type, forKey key: StringCodingKey) throws -> String {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: Double.Type, forKey key: StringCodingKey) throws -> Double {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: Float.Type, forKey key: StringCodingKey) throws -> Float {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: Int.Type, forKey key: StringCodingKey) throws -> Int {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: Int8.Type, forKey key: StringCodingKey) throws -> Int8 {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: Int16.Type, forKey key: StringCodingKey) throws -> Int16 {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: Int32.Type, forKey key: StringCodingKey) throws -> Int32 {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: Int64.Type, forKey key: StringCodingKey) throws -> Int64 {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: UInt.Type, forKey key: StringCodingKey) throws -> UInt {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: UInt8.Type, forKey key: StringCodingKey) throws -> UInt8 {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: UInt16.Type, forKey key: StringCodingKey) throws -> UInt16 {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: UInt32.Type, forKey key: StringCodingKey) throws -> UInt32 {
        try singleValueContainer(for: key).decode(type)
    }

    func decode(_ type: UInt64.Type, forKey key: StringCodingKey) throws -> UInt64 {
        try singleValueContainer(for: key).decode(type)
    }
    ...
    func decodeNil(forKey key: StringCodingKey) throws -> Bool {
        let encodedKey = configuration.keyStrategy.encode(key, at: codingPath)
    ...
    func nestedUnkeyedContainer(forKey key: StringCodingKey) throws -> UnkeyedDecodingContainer {
        switch try childContainer(for: key) {
        case let .multiValue(multiValueContainer):
    ...
    func superDecoder(forKey key: StringCodingKey) throws -> Decoder {
        let encodedKey = configuration.keyStrategy.encode(key, at: codingPath)
